...
# RPM-family package-manager user adjustment, if such a user exists.
grep -q '^_dnf:' /etc/passwd && usermod -g aid_inet _dnf || true
...
# Final cleanup.
RUN rm -rf /var/cache/dnf /var/lib/dnf /tmp/* /var/tmp/* || true
